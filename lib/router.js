/**
 * router主模块
 * create by wanghx   https://github.com/whxaxes/easy-router
 * 原作者博客 http://www.cnblogs.com/axes/p/4491983.html
 * 修改 vinc,2016.7.6
 */
...
rp.route = function(req, res) {
  ...
  logger.log(logger.color('404 get ' + reqUrl, 'gray'));
...
rp._processUrl = function(req, res, fil, ads, suitpath) {
  ...
  if (fs.lstatSync(filepath).isDirectory()) {
    if (this.debug) {
      ...
      logger.log(logger.color('200 get ' + suitpath, 'green'));
...
rp.routeTo = function(req, res, filepath, headers) {
  ...
  var etag, times, stats;

  if (!fs.existsSync(filepath)
    || !(stats = fs.statSync(filepath)).isFile()) return;

  var fileKind = path.extname(filepath).toLowerCase();
  var source = fs.createReadStream(filepath);

  //设置响应头信息
  if (fileKind.substr(0,1)=='.')
  fileKind = fileKind.substr(1);
  var type = mime.types[fileKind] || 'text/plain';
  var options = {
  ...
  if (that.useCache && cacheAllow.indexOf(type) >= 0) {
    ...
    if (req.headers['if-none-match'] === etag) {
      that.cache(res);
      logger.log(logger.color('304 cache ' + filepath, 'yellow'));
      return true;
    }
  ...
  logger.log(logger.color('200 get ' + req.url, 'green'));
...
rp._getFolderList = function(folderpath, re, grepKey) {
  ...
  function getlist(fpath, prefix) {
    ...
    _.each(folders, function(f) {
      ...
      if (fs.existsSync(npath) && fs.lstatSync(npath).isDirectory()) {
