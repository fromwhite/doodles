...
const getFragmentShader = (iterations: number, useCustomColors: boolean) => `
  ...
  vec4 aurora(vec3 ro, vec3 rd) {
      ...
      for(float i=0.; i < ${iterations.toFixed(1)}; i++) {
          ...
          ${
            useCustomColors
              ? `col2.rgb = mix(colorA, colorB, sin(i * 0.043) * 0.5 + 0.5) * rzt;`
              : `col2.rgb = (sin(1. - vec3(2.15, -0.5, 1.2) + i * 0.043) * 0.5 + 0.5) * rzt;`
          }
...
export const Aurora = ({
  ...
  const CustomMaterial = useMemo(() => {
    const M = shaderMaterial(
      ...
      getVertexShader(),
      getFragmentShader(iterations, !!colors),
    );
    extend({ AuroraMaterial: M });
    return M;
  }, [iterations, !!colors]);

  const [w, h] = useMemo(() => [size.width * 0.9, size.height * 0.84], [size]);
  const resolution = useMemo(() => {
    const factor = lowRes ? 0.5 : 1.0;
  ...
  useFrame((state, delta) => {
    ...
    mat.iMouse.lerp(
      new THREE.Vector2(
        (state.mouse.x * 0.5 + 0.5) * w * factor,
        (state.mouse.y * 0.5 + 0.5) * h * factor,
      ),
      0.05,
    );
  });
  ...
  return (
    <mesh ref={meshRef}>
      ...
      <auroraMaterial
        key={`${CustomMaterial.key}-${quality}-${lowRes}-${!!colors}`}
        iResolution={resolution}
        colorA={colors ? new THREE.Color(colors[0]) : undefined}
        colorB={colors ? new THREE.Color(colors[1]) : undefined}
      />
    </mesh>
  );
};
